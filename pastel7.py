...
def generate_seamless_dots_and_lines_abstract(width, height, dpi, density_factor, dot_size_range, line_thickness_range, color_scheme_name):
    colors = get_pastel_color_scheme(color_scheme_name)
    bg_color = colors[0] if colors else (255, 255, 255)
...
# --- Fungsi Baru: Menghasilkan Pola Geometris Modular Seamless ---
def generate_seamless_modular_geometry(width, height, dpi, grid_size, shape_type, shape_size_range, rotation_angle, color_scheme_name):
    colors = get_pastel_color_scheme(color_scheme_name)
    bg_color = colors[0] if colors else (255, 255, 255)
    element_colors = colors[1:] if colors and len(colors) > 1 else [(0, 0, 0)]
    ...
    # Use grid_size directly for modularity
    buffer = grid_size * 2
    extended_width = width + buffer * 2
    extended_height = height + buffer * 2
    ...
    for x_grid in range(-buffer, extended_width, grid_size):
        for y_grid in range(-buffer, extended_height, grid_size):
            ...
            # Ensure shape stays within cell boundaries or is designed to overlap intentionally
            # For simplicity, we'll draw centered shapes, then wrap
...
selected_pattern_type = st.sidebar.selectbox(
    "Pilih Tipe Pola:",
    ("Polka Dot", "Garis-garis (Horizontal/Vertikal)", "Garis-garis (Diagonal)", "Segitiga", "Kotak/Wajik", "Kombinasi Modern (Dots & Lines)", "Geometris Modular")
)
...
if selected_pattern_type == "Polka Dot":
    ...
    diag_angle = st.sidebar.slider("Sudut Diagonal (derajat)", 1, 89, 45, 1)
    ...
    with st.spinner("Menggenerasi pola, mohon tunggu..."):
        ...
        if selected_pattern_type == "Polka Dot":
            ...
            generated_image = generate_seamless_dots_and_lines_abstract(
                CANVAS_WIDTH, CANVAS_HEIGHT, OUTPUT_DPI,
                density_factor, dot_size_range_modern, line_thickness_range_modern, actual_color_scheme_key
            )
            filename = f"dots_lines_abstract_{actual_color_scheme_key}.jpg"
        elif selected_pattern_type == "Geometris Modular":
            generated_image = generate_seamless_modular_geometry(
                CANVAS_WIDTH, CANVAS_HEIGHT, OUTPUT_DPI,
                grid_cell_size, modular_shape_type, modular_shape_size_range, modular_rotation_angle, actual_color_scheme_key
            )
